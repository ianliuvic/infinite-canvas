...
需要将现有图片节点持久化、修复产品外链的跨域问题且不加入资产库时，使用 canvas_persist_external_images。传入 nodes:[{nodeId,url?}]；有可用 HTTPS content 时可省略 url，失效 blob 必须先找到真实来源再提供 url，不得猜测。这个工具更新原节点，不需要 assets_add，不新增节点，不使用浏览器下载，也不生图。等工具返回保存成功再报告完成；失败时不要重复创建资产或节点。
