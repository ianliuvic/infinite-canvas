...
export const toolInputSchemas = {
    ...
    canvas_persist_external_images: z.object({ nodes: z.array(z.object({ nodeId: z.string(), url: z.string().url().optional() })).min(1) }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_apply_ops: "批量操作当前网页画布。ops 支持 add_node、update_node、delete_node、delete_connections、connect_nodes、set_viewport、select_nodes、run_generation。新增节点时除非用户明确指定精确坐标，否则使用 autoPosition: true 并省略 position/x/y，让画布自动避让。",
    canvas_create_node: "创建任意类型节点：text、image、config、video、audio、group。适合创建占位图、媒体占位、配置节点、分组或自定义 metadata 节点；除非用户明确要求精确位置，否则省略 x/y。",
    canvas_create_attachment_nodes: "把当前对话中用户上传的图片或视频附件创建成真实画布媒体节点。attachmentIds 使用本轮附件清单中的 ID；图片节点 ID 可传给 canvas_create_generation_flow.referenceNodeIds 作为生成参考图。",
    canvas_persist_external_images: "将现有图片节点的外链由服务端下载到 R2 并更新原节点 storageKey。不加入资产库，不新增节点、不改变位置分组连线，不生图。nodes 指定 nodeId，可用 url 提供原始 HTTPS 图片地址（修复失效 blob 时必填，不能猜测来源）。已有 storageKey 且未指定新 url 时验证并复用。默认允许 wearhongxiu.com 来源，其他域名需管理员配置。等待网页保存确认才完成。",
    canvas_create_text_node: "在当前画布创建单个文本节点；除非用户明确要求精确位置，否则省略 x/y。",
    ...
    canvas_create_generation_flow: "创建通用生成流程：提示词文本节点、生成配置节点、参考节点连线，可用于文案、生图、视频或音频。默认省略 x/y，让画布沿引用节点右侧自动避让排列。",
