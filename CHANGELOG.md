...
+ [修复] 删除画布组时同步删除组内全部节点及其连线，不再只移除外侧组框。

+ [新增] 保存冲突提示新增“放弃本机冲突”入口，可在确认后清除冲突待同步记录并重新载入云端版本。

+ [修复] Crun 生图改为持久化后台任务并在画布中保存任务 ID，同时强制托管 Crun 通道绕过浏览器遗留的同步脚本；Cloudflare 短暂错误或页面刷新后继续查询原任务且不重复提交扣费。

+ [新增] Agent 支持将现有节点的业务站点外链图片下载到 R2 并原位更新，不创建资产库条目或额外节点。

+ [修复] 从资产库或 Agent 添加实体组、图片和视频时先根据持久化键恢复当前页面可用地址，避免插入旧 blob 地址导致必须刷新才能显示。

+ [修复] 托管画布增加刷新可恢复的持久化待同步队列、并发三方合并和可见保存状态；Agent 等待云端确认，禁止浏览器依据局部状态清理云端素材。

+ [修复] 我的资产页面深度刷新时正确返回应用入口，避免与静态资源目录重名后跳转到内部 3000 端口；缺失的编译资源返回 404。

+ [新增] 画布节点和组支持非破坏性禁用；禁用项保留节点与连线，但不再作为生成或 Agent 引用输入，重新启用后可继续使用。
+ [优化] 新生成的图片节点最大显示尺寸提升到 480×480，视频节点提升到 560×560，竖版和横版素材均按原始比例使用更大的画布空间。
+ [新增] 画布图片与视频双击预览支持左右按钮和键盘方向键连续浏览当前画布中的媒体节点。
+ [修复] 服务器托管的模型通道不再向同源网关发送占位 Bearer 请求头，避免覆盖站点 HTTP Basic Auth 并在运行节点时反复弹出登录框；该规则统一适用于后续接入的非 Crun 托管模型。
+ [新增] Coolify 部署支持通过运行期环境变量启用站点级账号密码保护，画布页面和 Agent 接口统一受保护且凭据不进入前端产物，并保留独立健康检查端点。
+ [修复] 复制配置节点时不再保留失效的提示词节点 ID，批量复制会同步重映射引用；生成时也会用当前连线补回未被有效占位符引用的文本和媒体。
+ [修复] Grok 单图图生视频会在提交前把参考图无裁切适配到所选输出比例，避免模型忽略 `aspect_ratio` 后沿用横版输入比例。
+ [新增] 画布底部新增基于 ELK 分层布局的一键整理，支持整理整个画布或多选节点，保持实体资产组内部结构并在整理后自动适配视图。
+ [优化] 右侧 Agent 新建节点改为沿选中节点或引用节点向右放置，并使用可扩展空位搜索避免节点增多后退回视口中央重叠。
+ [修复] Crun 视频调用脚本不再重复拼接 `/v1`，Worker 同时兼容已打开页面缓存的旧路径，避免创建与查询任务显示 `not found`。
+ [修复] Crun 视频生成的参考图上传、估价、任务创建与生成均改为 Worker 后台执行，避免任一阶段的长请求被 Cloudflare 中断，同时在失败节点中显示 Crun 返回的具体错误而非通用 HTTP 502。
+ [修复] 右侧 Agent 的停止按钮现在可取消排队、附件写入与视频解析阶段的任务，并在 Codex turn 已启动后继续使用原生中断，不再出现按钮可见却提示没有可停止任务。
+ [修复] 右侧 Agent 在浏览器切到后台或短暂断网后不再因连续 SSE 错误永久关闭连接，返回画布时会继续自动重连并从服务端快照恢复当前会话与任务状态。
+ [优化] 资产页不再在 hydration 阶段下载全部 R2 图片，实体卡片、单项资产和画布素材选择器改为进入视口后按需加载并复用本地缓存。
+ [修复] 已持久化的实体图片恢复时始终优先使用 R2 对象，不再因资产仍保留外部来源 URL 而绕过持久化副本。
+ [新增] 右侧 Agent 支持上传常见视频文件，由 Worker 读取视频元数据并抽取代表帧交给当前所选模型进行视觉分析，同时可将原视频持久化为画布视频节点。
+ [修复] 远端状态写入增加乐观锁版本校验，页面刷新、部署期间读取失败或并发旧标签页不再以空白或过期状态覆盖 PostgreSQL 中的资产和画布数据。
+ [新增] 右侧 Agent 支持上传 PDF、TXT、Markdown 和 JSON 文本文件，以文件卡片展示并作为当前 Codex 任务可直接读取的工作区附件。
+ [优化] 打开画布时不再重复上传浏览器中已有的图片和媒体，远程素材暂时不可用时也不会阻塞画布初始化。
+ [新增] 图片、视频、音频和文本节点工具栏可一键“发给 Agent”，自动打开右侧输入框并插入可预览、可移除且保留节点 ID 的画布素材引用。
+ [修复] Crun 参考图生成优先从持久化存储恢复图片；生成配置会实际传递所有已连接媒体，不再把可选媒体字段发送为 `null`；同时严格校验媒体类型、等待上传文件可读并保留具体错误阶段。
