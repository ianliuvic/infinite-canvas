...
- Crun 等长耗时生成必须先返回并持久化本地任务 ID，再由短请求查询状态；页面刷新只能继续查询原任务，不得重新提交可能重复扣费的生成请求。

- 资产插入节点之前必须通过 storageKey 解析当前页面可用媒体地址；不得直接复用持久化 JSON 中的 blob URL，也不得依赖刷新时的 hydration 修复插入结果。手动插入和 Agent 插入必须遵循同一规则。

- 托管部署使用 PostgreSQL 状态和 R2 媒体；仅非托管部署默认浏览器本地存储。任何保存改动必须保留持久化 outbox，云端确认前不得显示“已保存”或向 Agent 返回持久化成功；遇到 409 必须三方合并或保留待同步冲突，刷新不得用远端旧数据覆盖未提交内容。
- 浏览器只掌握局部引用，不得自动删除托管 R2 对象。恢复或清理必须保护其他标签页、待同步记录及历史版本；不得以“当前页面未引用”为删除依据。
